<template>
  <div>
    <div>
      <button class="primary" @click="() => $state.createUser()">
        New User
      </button>
    </div>
    <data-table
      :rows="$state.users"
      :columns="['id', 'username', 'online', 'controls']"
    >
      ...
      <template v-slot:col-controls>&nbsp;</template>
      ...
      <template v-slot:row-online="{ row }">
        <div
          class="indicator"
          :class="{ online: row.online, offline: !row.online }"
        ></div>
      </template>
      <template v-slot:row-controls="{ row }">
        <a v-if="!row.wizard" :href="loginUrl(row.id)" target="_blank">Login</a>
      </template>
    </data-table>
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    loginUrl(userId: string) {
      return `${location.href.replace(/\/editor.*/, '')}?id=${userId}`
    },
  },
})
</script>
