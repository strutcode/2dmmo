<template>
  <div class="layout-h layout-fill" style="min-height: 0">
    <sprite-list class="chrome panel" />
    <div class="chrome panel layout-v layout-fill">
      <div v-if="$state.currentSprite">
        <h2>Source</h2>
        <div>
          <drop-down v-model="$state.currentSprite.set">
        ...
        <data-table
          :columns="['preview', 'name', 'x', 'y', 'frames', 'fps', 'loop', 'next']"
        ...
        >
          ...
          <template v-slot:row-next="{ row }">
            <input type="text" size="2" v-model="row.anim.next" />
          </template>
        </data-table>
      </div>
    </div>
...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    computed: {
      computedAnimations() {
        const sprite = this.$state.currentSprite

        if (!sprite) {
          return []
        }

        return sprite.animations.map((anim) => {
          return {
            preview: enemies[sprite.set],
            anim,
          }
        })
      },
    ...
    methods: {
      newAnim() {
        if (!this.$state.currentSprite) return

        this.$state.currentSprite.animations.push({
          name: '',
          x: 0,
          y: 0,
          frames: 4,
